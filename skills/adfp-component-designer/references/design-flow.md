...
从输入中提取用户体验要求，为每个组件识别四种状态处理策略。

### 交互场景提取

从 PRD、需求描述或 SPEC 中提取用户与系统的交互场景：

```
用户路径：入口 → 操作A → 分支判断 → 操作B → 结果
异常路径：操作A → 无数据 → 空状态提示
错误路径：操作A → 请求失败 → 错误提示 + 重试
```

### 四态识别

对清单中的**每个组件**逐一标注四种状态的处理方式：

| 组件 | loading | empty | error | success |
|------|---------|-------|-------|---------|
| {组件名} | Skeleton / Spin / Loading | 空状态提示+引导 | 错误提示+重试 | Toast/反馈 |

### 交互反馈模式推荐

| 场景 | 推荐反馈 | 说明 |
|------|---------|------|
| 列表加载 | Skeleton 骨架屏 | 占位显示结构，避免布局跳动 |
| 按钮提交 | Button loading + 禁用 | 防止重复提交 |
| 操作成功 | Toast/Message 轻提示 | 不阻断操作流程 |
| 页面切换 | 进度条或过渡动画 | 减少等待感知 |
| 下拉刷新 | PullRefresh + 加载指示器 | 移动端常见交互 |
| 请求失败 | 错误提示 + 重试按钮 | 给用户修复路径 |
| 空列表 | 插画/图标 + 引导文案 + 操作按钮 | 引导用户填充内容 |
